...
| `COREMETRY_CH_INSERT_DISTRIBUTED_SYNC` | `config.go` | `false` | **Geçici köprü** (v0.10.778): Distributed `spans`/`logs`/`metric_points` INSERT'leri senkron (`insert_distributed_sync=1`, `insert_distributed_timeout=60`); satırlar shard'lara sorgu içinde gider, yerel spool'a yazılmaz. Gönderici asılıyken DBA restart'ına kadar yeni verinin spool'da hapsolmasını önler. Bedel: hedef shard erişilmezse INSERT hata verir (`write_failed`), spool'a düşmez; ingest insert'i shard MV kaskadını bekler. Kalıcı ayar olarak önerilmez. | ingest/all | evet |
