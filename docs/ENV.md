...
| `COREMETRY_CH_READ_MAX_REPLICA_DELAY` | `config.go` | kod `0` (CH varsayılanı 300); **imajda `60`** (Dockerfile, v0.10.790) | Saniye. Distributed SELECT'te replikasyon gecikmesi eşiği aşan replika seçimden düşer (`max_replica_delay_for_distributed_queries`); tümü aşmışsa bayat olan yine kullanılır (erişilebilirlik). AYRI ZooKeeper yolunda yaşayan replikalara çare değil (küme yapılandırması; Admin → Replika tutarlılığı). Tek düğümde etkisiz. | api/worker/ingest | evet |
| `COREMETRY_CH_MV_DEDUP_BLOCKS` | `config.go` | kod `false`; **imajda `1`** (Dockerfile, v0.10.790) | `deduplicate_blocks_in_dependent_materialized_views=1`: zaman aşımı sonrası yeniden denenen AYNI blok ham tabloda tekilleşir, MV'lerde de tekilleşsin — MV sayımı ham'ı aşmasın. Tek düğümde de geçerli (Replicated değilse etkisiz). | ingest/all | evet |
| `COREMETRY_CH_INSERT_QUORUM` | `config.go` | `0` (kapalı; imajda YOK) | ≥2 verilirse INSERT en az N replikaya yazılana dek döner (`insert_quorum`, `insert_quorum_parallel=1`, 60 sn). Replika düşükken INSERT hata verir (`write_failed`); erişilebilirlik bedeli olduğu için bilinçli opt-in. | ingest/all | evet |
| `COREMETRY_CH_READ_LOAD_BALANCING` | `config.go` | `""` → CH varsayılanı (`random`); **imajda YOK** (v0.10.822) | CH `load_balancing`: Distributed SELECT'te shard başına hangi replikanın seçileceği. İzinli: `in_order` · `first_or_random` · `random` · `nearest_hostname` · `round_robin`; başka değer → boot'ta `[chstore] WARNING` + yok sayılır (izin listesi tek gövdede: `chstore/read_balancing.go`). **Yalnız okuma havuzu + ana bağlantı; ingest havuzuna uygulanmaz** (aynı ayar Distributed INSERT'te de replika seçer). Belirti-çözüm: aynı sorgu her yenilemede farklı sayı → **`in_order` + `COREMETRY_CH_READ_PREFER_LOCALHOST=0` İKİSİ BİRLİKTE** (yalnız `in_order` yetmez, bkz. alttaki satır). Sayıyı SABİTLER ama **ıraksamayı gizler, veriyi düzeltmez** — ölçüm Admin → ClickHouse → Replika tutarlılığı, onarım Replika onarımı sihirbazı. Etkin değer Replika kartındaki `load_balancing=` çipinde ve boot logunda görünür. Tek düğümde etkisiz. | api/worker/ingest | evet |
| `COREMETRY_CH_READ_PREFER_LOCALHOST` | `config.go` | ayarsız → ayar gönderilmez = **CH varsayılanı `1`**; imajda YOK (v0.10.822) | Üç durumlu: boş = CH varsayılanı (`1`), `0/false/off/no` → `prefer_localhost_replica=0`, `1/true/on/yes` → `1`. Başka değer → `[chstore] WARNING` + yok sayılır. **Determinizm için `0` ŞART:** varsayılan `1`, koordinatörün kendi barındırdığı shard'ı `load_balancing`'e hiç bakmadan yerelden okutur; okuma havuzu RoundRobin olduğu için koordinatör her sorguda değişir ve sayı yalnız `in_order` ile de zıplamaya devam eder. **Yalnız okuma havuzu + ana bağlantı; ingest havuzuna uygulanmaz** (`0` yazma yolunda yerel sink'i kapatır, INSERT'leri shard başına ilk replikaya hunilerdi). Aynı uyarı: ayrışmayı gizler, düzeltmez (Admin → Replika tutarlılığı/onarımı). Tek düğümde etkisiz. | api/worker/ingest | evet |
